...
//            ->group('lnc_post_title.title,lnc_post_content.content,lnc_post.p_date,lnc_user.u_name')
            ...
            ->field('lnc_post.p_id,lnc_post_title.title,lnc_post_content.content,lnc_post.p_date,lnc_user.u_name')
            ->select();
//        //将每条数据的时间戳转换成日期格式
//        foreach ($Post as &$value){
//            $value['p_date']=date("Y-m-d",$value['p_date']);
//        }
        //赋值
        $this->assign('post',$Post);

        //获取帖子数量资源
        $Post_count = $post->where("b_id=$b_id")->count();
        //赋值
        ...
        //获取贴吧被关注数量
        $Attention_count = $attention->where("b_id=$b_id")->count();//获取关注数量
        //赋值
